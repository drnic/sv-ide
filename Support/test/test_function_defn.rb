...
class TestFunctionDefn < Test::Unit::TestCase
  should "load function definition with no parameters" do
    func = FunctionDefn.find_by_name("treRollback&").values.first
    assert_not_nil(func)
    assert_equal("treRollback&", func.name)
  ...
  should "load function definition with parameters" do
    ...
    assert_equal(%w[ReferenceTypeLabel$ ReferenceCodeLabel$], func.parameters)
  end
  ...
  should "load function and have correct name" do
    ...
    assert_equal("SQLQuery?[]", FunctionDefn.find_by_name("SQLQuery?[]").values.first.name)
  end
  
  should "find multiple interfaces" do
    interfaces = FunctionDefn.find_by_name("SQLQuery&")
